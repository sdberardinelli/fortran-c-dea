...
{
    //printf("allocMYBOOL: lp addr 0x%0X\n", lp);
  if(clear == TRUE)
  {
      //printf("allocMYBOOL: 1\n");
      *ptr = (MYBOOL *) calloc(size, sizeof(**ptr));
  }
  else if(clear & AUTOMATIC) {
        //printf("allocMYBOOL: 2\n");
  ...
  //printf("allocMYBOOL: 4\n");

  if(((*ptr) == NULL) && (size > 0)) {
        //printf("allocMYBOOL: 5\n");
