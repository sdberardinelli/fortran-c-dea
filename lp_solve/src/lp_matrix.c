...
{
//    printf("mat_appendrow: mat addr 0x%0X\n", mat);
//    printf("mat_appendrow: row addr 0x%0X\n", row);
  ...
  if(checkrowmode && mat->is_roworder)
  {
//      printf("mat_appendrow: 1\n");
    return( mat_appendcol(mat, count, row, colno, mult, FALSE) );
  }



//    printf("mat_appendrow: 2\n");

  /* Do initialization and validation */
  isA = (MYBOOL) (mat == lp->matA);
  isNZ = (MYBOOL) (colno != NULL);
  if(isNZ && (count > 0)) {
//        printf("mat_appendrow: 2a\n");
    if(count > 1)
    {
//        printf("mat_appendrow: 2c\n");
        sortREALByINT(row, colno, count, 0, TRUE);
    }

    if((colno[0] < 1) || (colno[count-1] > mat->columns))
    {
//        printf("mat_appendrow: 2d\n");
      return( 0 );
    }
...
//printf("mat_appendrow: 4\n");
  /* Optionally tally and map the new non-zero values */
  firstcol = mat->columns + 1;
  if(isNZ) {
//        printf("mat_appendrow: 4a\n");
    newnr = count;
    if(newnr) {
      firstcol = colno[0];
      jj = colno[newnr - 1];
...
//      printf("mat_appendrow: 4b\n");
    newnr = 0;
    if(row != NULL) {
//        printf("mat_appendrow: 4c\n");
      if(!allocMYBOOL(lp, &addto, mat->columns + 1, TRUE)) {
//            printf("mat_appendrow: exit\n");
        return( newnr );
      }

//      printf("mat_appendrow: 4d\n");
//      printf("mat_appendrow: 4d %d\n", mat->columns);
//       printf("mat_appendrow: 4d %f\n", mat->epsvalue);
...
//    printf("mat_appendrow: 4e\n");
  }

//printf("mat_appendrow: 5\n");
...
{
//    printf("mat_appendcol: mat addr 0x%0X\n",mat);
